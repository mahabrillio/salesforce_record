...
export class SalesforceClient {
    ...
    fetchChildRecords(recordId, optExistingSchema) {
        const recordUiUrl = `${this.apiUrl_}/${RECORD_UI_ENDPOINT}/${recordId}`;
        console.log(recordUiUrl);
        const data = {layoutTypes: "Full",childRelationships:"Opportunity.FinancialAccountApplicationsFSC__r"};
        return this.request_("GET", recordUiUrl, data).then(response => {

            console.log(response.records[recordId].childRelationships);
            let childCount = response.records[recordId].childRelationships.FinancialAccountApplicationsFSC__r.count;
            let childrecords = response.records[recordId].childRelationships.FinancialAccountApplicationsFSC__r.records;

            return [childCount, childrecords];
        });
    }
